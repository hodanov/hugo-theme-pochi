...
const specs = [
  {
    ...
    src: path.join(projectRoot, 'node_modules', 'fuse.js', 'dist', 'fuse.min.js'),
  ...
  },
  {
    ...
    src: path.join(projectRoot, 'node_modules', 'mark.js', 'dist', 'mark.min.js'),
...
function sync() {
  ...
  for (const s of specs) {
    ...
    console.log(`Synced ${s.name} -> ${path.relative(projectRoot, s.dest)} (${hash.slice(0,8)}…)`);
  }
  writeLock(lock);
}
...
function verify() {
  ...
  for (const s of specs) {
    ...
    const h = sha256(buf);
    if (h !== meta.sha256) {
      console.error(`Hash mismatch for ${rel}: lock=${meta.sha256} actual=${h}`);
      ok = false;
    }
    if (s.version && meta.version && s.version !== meta.version) {
      console.error(`Version mismatch for ${rel}: lock=${meta.version} pkg=${s.version}`);
      ok = false;
    }
  }
