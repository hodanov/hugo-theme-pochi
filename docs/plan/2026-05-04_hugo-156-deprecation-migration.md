...
- [x] `hugo build --logLevel info` の出力に `deprecated` を含む行が無い
- [x] `make server` 起動時、トップページ・記事ページ双方の `<head>` に hreflang が言語数ぶん出る
- [x] 翻訳が存在しないページで言語スイッチャーが全言語のエントリを表示する
- [x] `x-default` の href が `defaultContentLanguage` の URL を指している
- [x] `/robots.txt` に `/ja/index.json` `/en/index.json` 等が漏れなく Disallow されている
- [x] `make build` が成功し、`hugo/public/` 配下の出力に差分が想定範囲内
- [x] 既存 lint（textlint / markdownlint）が通る
