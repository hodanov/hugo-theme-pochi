...
- [x] 言語指定ありのコードブロック（`js,`python 等）でコピーボタンが表示される
- [x] 言語指定なしのコードブロックでもコピーボタンが表示される
- [x] コピーボタンクリックでクリップボードにコードがコピーされる
- [x] コピー成功時にフィードバック（テキスト変化）が 2 秒間表示される
- [x] PJAX ナビゲーション後もコピーボタンが正常に動作する
- [x] ダークモード・ライトモード両方でボタンが視認できる
- [x] モバイル（タッチデバイス）でボタンが操作可能
- [ ] `prefers-reduced-motion` 設定時にアニメーションがスキップされる
- [x] 日本語・英語の両方でフィードバックテキストが正しく表示される
- [x] markdownlint / ESLint でエラーが出ない
